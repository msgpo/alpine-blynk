...
MOUNTFLAGS := # -v $(CURDIR)/data:/opt/blynk/data
...
PORTFLAGS  := -p 7443:7443 -p 8080:8080 -p 8081:8081 -p 8082:8082 -p 8441:8441 -p 8442:8442 -p 8443:8443 -p 9443:9443
...
test :
	...
	if [ "$(ARCH)" != "armhf" ]; then docker run --rm -it $(NAMEFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(MOUNTFLAGS) $(OTHERFLAGS) $(IMAGETAG) sh -ec 'sleep 5; java -version; cat /opt/blynk/version'; fi;
